...
public class SGWFileParserTests
{
    ...
    {
        var room = _parser.ParseRoom(Sgw(
            "@room 3001 \"A Street\"",
            "sector: city",
            "> north 3002",
            "@end"
    ...
    public void ParseRoom_ExtraContinuationLines_AreJoinedToInlineText()
    {
        ...
        Assert.That(room.Extras[0].Text, Is.EqualTo(
            "The door is a slab of iron,\ntaller than a man and twice as wide."));
    }
    ...
    {
        return result.Diagnostics.Select(d => d.Code);
    }

    private SGWRoomParseResult Parse(params string[] lines)
    {
        return _parser.TryParseRoom(Sgw(lines), "temple.sgw");
    }
    ...
    {
        var exception = Assert.Throws<SGWParseException>(() => _parser.ParseRoom(Sgw(
            "@room 3001 \"A Street\"",
            "sector: nowhere",
            "@end"
        )));
